...
def main(args):
    ...
    with open(args.output_file, 'w') as w:
        w.write("{} molecule sampled at random from {}\n".format(args.sample_size, args.input_file))
        mol_references = convert_smi_to_mol(references)
        w.write("Mean Levenshtein:{}\n".format(levenshtein_eval(references)))
        w.write("Mean BLEU-1:{}\n".format(bleu_eval(args, references)))
        w.write("Mean Morgan Fingergprint:{}\n".format(morgan_fingerprint_evaluation(mol_references)))
        w.write("Mean RDkit Fingergprint:{}\n".format(rd_fingerprint_evaluation(mol_references)))
        w.write("Mean MAACS Fingergprint:{}\n".format(maacs_fingerprint_evaluation(mol_references)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_file', type=str, default='outputs/metrics_random.txt')
